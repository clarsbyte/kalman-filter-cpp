...
kf::ExtendedKalmanFilter::ExtendedKalmanFilter(
...
void kf::ExtendedKalmanFilter::init() {

  P = P0;
  x_hat.setZero();
}

void kf::ExtendedKalmanFilter::init(const Eigen::VectorXd& x) {

  init();
  x_hat = x;
}

void kf::ExtendedKalmanFilter::predict(const Eigen::VectorXd& u) {
  Eigen::VectorXd x = x_hat;
  x_hat = f(x_hat, u);
  P = F(x, u) * P * F(x, u).transpose() + Q;
}

void kf::ExtendedKalmanFilter::update(const Eigen::VectorXd& y) {
	Eigen::VectorXd x = x_hat;
...
Eigen::VectorXd kf::ExtendedKalmanFilter::get_state() {
  return x_hat;
}

Eigen::MatrixXd kf::ExtendedKalmanFilter::get_covariance() {
  return P;
}
